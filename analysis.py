...
def get_states(config, model, data_loader, layer=-1):
    ...
    for _, batch in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            if config.model_name in ["bert", "distbert", "albert", "roberta"]:     
                states.append(torch.reshape(batch_states[layer][:,0,:], (-1,768)).detach().cpu().numpy()) 
            # Last layer representation for decoders
            if config.model_name in ["gpt2", "gpt2-medium", "pythia-70m", "pythia-160m", "pythia-410m"]:        
                states.append(torch.reshape(batch_states[layer][:,-1,:],(-1, batch_states[-1].shape[-1])).detach().cpu().numpy())    
...
def main():
    ...
    if config.analysis == "get_ci": 
        ...
        for i in range(1,12,1):
            config.layer = i
            var = get_ci(config, train_loader, model, 500000, gpu_id=None)
            print("LAYER", i)
